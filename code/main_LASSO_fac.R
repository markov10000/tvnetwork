...
maxlag <- max(lags)
  ...
  mvarmodel<- mvar(hat.X,type=rep("g",d),lags=lags)
  mvarmodel2<- rewLASSO(mvarmodel,hat.X)[[1]]
  #rm(comb)
  ...
  residuals <- get.residuals(data=hat.X, estpoints=estpoints_res, lags=c(1:1), coef=coef)
  residuals1 <-residuals[[1]]
  residuals2 <-residuals[[2]]
  perr <- get.predict.error(hat.X, lags, discard=discard, coef=coef)
  error1step1 <- perr[[1]]
  error1step2 <- perr[[2]]
  
  listC1 <- lapply(seq(dim(coef1)[4]), function(x) coef1[ , ,1, x])
  SST1 <- apply(hat.X[-1,],2,var)* (n-1)  
