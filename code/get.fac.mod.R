...
get.fac.mod <- function(x, max.q=NULL, q=NULL, bn.op=2, normalisation=FALSE){
	...
	if(is.null(max.q)) {
		max.q <- max(min(10,cnt),round(sqrt(cnt)))
	}
	else{
		if(max.q>cnt) max.q <- cnt
	}
	...
	xx <- t(x)%*%x/n
	if (T==max.q){
	  eig <- eigen(x=xx,symmetric=TRUE) 
	}else{
	  eig <- eigs_sym(A=xx,k=max.q) # only calculate first k eigen, using (rARPACK)
	}
