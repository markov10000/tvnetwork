...
constant5 <- 1
rrho1 <- 0.2
rrho2 <- 2 #rrho controls the smoothness of function. rrho = 0 means constant.
epsilonscale<- sqrt(1)
...
f1 <- rep(0,n)
f2 <- rep(0,n)
lam1 <- matrix(rnorm(d),d,n)
lam2 <- matrix(0,d,n)
com.com<-matrix(0,n,d)
u_f1 <- rnorm(n,mean=0,sd=sqrt(1-0.6^2))
u_f2 <- rnorm(n,mean=0,sd=sqrt(1-0.3^2))
f1[1]<-rnorm(1)
f2[1]<-rnorm(1)
for (t in (2:n))
{
  f1[t] <- 0.6*f1[t-1]+u_f1[t]
  f2[t] <- 0.3*f2[t-1]+u_f2[t]
  lam2[,t] <- 2/(1+exp(-2*(10*t/n-5*seq(1,d)/d-2)))
  com.com[t,] <- f1[t]*lam1[,t]+f2[t]*lam2[,t]
}
Y <- com.com + X
#source('localpca.R')
localpca.m <- localpca(Y)
print(localpca.m$hat.r)
hat.X<- localpca.m$hat.err
